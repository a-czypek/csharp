namespace Zestaw17Zadanie1
{
    internal class Program
    {
        public static List<Employees> EmployeesList;
        ...
        static void Main(string[] args)
        {
            ...
            // Zadanie 1
            // Z listy EmployeesList wybierz nazwisko i imię osób urodzonych w styczniu 1966 roku
            // Wyświetl uzyskany wynik

            var wynik1 = EmployeesList.Where(x => x.BirthDate.Year == 1966 && x.BirthDate.Month == 1);

            Console.WriteLine();
            foreach(var employee in wynik1)
            {
            ...
            // Zadanie 2
            // Z listy CustomersList wybierz kraj, miasto oraz nazwę firmy klientów z Kanady oraz Brazyli
            // Wyświetl uzyskany wynik

            var wynik2 = CustomersList.Where(x => x.Country == "Canada" || x.Country == "Brazil");

            Console.WriteLine();
            foreach(var customer in wynik2)
            {
            ...
            // Zadanie 3
            // Korzystając z danych zapamietanych w liście EmployeesList ustal w których miastach zatrudnionych jest 
            // więcej niż jeden pracownik
            // Wyświetl uzyskany wynik

            var wynik3 = EmployeesList.GroupBy(x => x.City).Where(g => g.Count() > 1).Select(g => new {klucz = g.Key, ile = g.Count()});

            Console.WriteLine();
            foreach(var employee in wynik3)
            {
                Console.WriteLine(employee.klucz + " " + employee.ile);
            }
            Console.WriteLine();


            // Zadanie 4
            // Korzystając z danych zapamietanych w liście CustomersList ustal liczbę klientów z poszczegulnych miast
            // Wyświetl uzyskany wynik

            var wynik4 = CustomersList.GroupBy(x => x.City).Select(x => new {klucz = x.Key, ile = x.Count()});

            Console.WriteLine();
            foreach(var x in wynik4)
            {
                Console.WriteLine(x.klucz + " " + x.ile);
            }
            Console.WriteLine();


            // Zadanie 5
            // Z listy EmployeesList wybierz nazwisko, imię, miasto pracowników zatrudnionych
            // na stanowisku "Sales Representative". Wynik uporządkuj malejąco według nazwiska 
            // Wyświetl uzyskany wynik

            var wynik5 = EmployeesList.Where(x => x.Title == "Sales Representative").Select(x => new {imie = x.FirstName, name = x.LastName, tytul = x.Title});

            Console.WriteLine();
            foreach(var employee in wynik5)
            {
                Console.WriteLine(employee.imie + " " + employee.name + " " + employee.tytul);
            }
            Console.WriteLine();


            // Zadanie 6
            // Z listy CustomersList wybierz dane klientów o imieniu Maria. Wynik uporządkuj malejaco według
            // nazwy miasta
            // Wyświetl uzyskany wynik

            var wynik6 = CustomersList.Where(x => x.ContactName.StartsWith("Maria")).Select(x => new
            {
                imie = x.ContactName,
                nazwa = x.CompanyName,
                miasto = x.City
            }).OrderByDescending(x => x.miasto);

            Console.WriteLine();

            foreach(var x in wynik6)
            {
                Console.WriteLine(x.imie + " " + x.nazwa + " " + x.miasto);
            }

            Console.WriteLine();

            // Zadanie 7
            // Korzystając z danych zapamietanych w liście EmployeesList ustal jaka jest srednia wieku 
            // pracowników
            // Wyświetl uzyskany wynik

            var wynik7 = EmployeesList.Select(e => (DateTime.Now - e.BirthDate).Days / 365.25).Average();

            Console.WriteLine("Zadanie 7:");
            Console.WriteLine($"Sredni wiek: {wynik7}");

            // Zadanie 8
            // Korzystając z danych zapamietanych w liście CustomersList ustal liczbę klientów których identyfikatory
            // rozpoczynają się od kolejnych liter alfabetu. Wynik uporzadkuj malajaco według liczby klientów
            // Wyświetl uzyskany wynik

            var zadanie8 = CustomersList.GroupBy(c => c.CustomerID[0]).Select(g => new { Initial = g.Key, Count = g.Count() }).OrderByDescending(g => g.Count);

            Console.WriteLine();
            Console.WriteLine("Zadanie 8:");
            foreach (var group in zadanie8)
            {
                Console.WriteLine($"{group.Initial}: {group.Count}");
            }
        ...
        public static void zaladujDane()
        {
            ...
            EmployeesList.Add(new Employees(2, "Fuller", "Andrew", "Vice President, Sales", "Dr.", new DateTime(1952, 2, 19), new  DateTime(1992,8,4), "908 W. Capital Way", "Tacoma", "WA", "98401", "USA", "(206) 555-9482", "3457"));
            EmployeesList.Add(new Employees(3, "Leverling", "Janet", "Sales Representative", "Ms.", new DateTime(1963, 8, 30), new DateTime(1992, 4, 01), "722 Moss Bay Blvd.", "Kirkland", "WA", "98033", "USA", "(206) 555-3412", "3355"));
            EmployeesList.Add(new Employees(4, "Peacock", "Margaret", "Sales Representative", "Mrs.", new DateTime(1937, 9, 19), new DateTime(1993, 5, 3), "4110 Old Redmond Rd.", "Redmond", "WA", "98052", "USA", "(206) 555-8122", "5176"));
            EmployeesList.Add(new Employees(5, "Buchanan", "Steven", "Sales Manager", "Mr.", new DateTime(1955,3,4), new DateTime(1993,10,17), "14 Garrett Hill", "London", "XX", "SW1 8JR", "UK", "(71) 555-4848", "3453"));     
            EmployeesList.Add(new Employees(6, "Suyama", "Michael", "Sales Representative", "Mr.", new DateTime(1963,07,02), new DateTime(1993,10,17), "Coventry House\nMiner Rd.", "London", "XX", "EC2 7JR", "UK", "(71) 555-7773", "428")); 
            EmployeesList.Add(new Employees(7, "King", "Robert", "Sales Representative", "Mr.", new DateTime(1960,5,29), new DateTime(1994 ,1,02), "Edgeham Hollow\nWinchester Way", "London", "XX", "RG1 9SP", "UK", "(71) 555-5598", "465"));     
            EmployeesList.Add(new Employees(8, "Callahan", "Laura", "Inside Sales Coordinator", "Ms.", new DateTime(1958,01,09), new DateTime(1994,03,05), "4726 - 11th Ave. N.E.", "Seattle", "WA", "98105", "USA", "(206) 555-1189", "2344"));  
            EmployeesList.Add(new Employees(9, "Dodsworth", "Anne", "Sales Representative", "Ms.", new DateTime(1966,1,27), new DateTime(1994 ,1,15), "7 Houndstooth Rd.", "London", "XX", "WG2 7LT", "UK", "(71) 555-4444", "452"));  
            EmployeesList.Add(new Employees(1, "Davolio", "Nancy", "Sales Representative", "Ms.", new DateTime(1948, 2,8), new DateTime(1992 ,5,1), "507 - 20th Ave. E.\nApt. 2A", "Seattle", "WA", "98122", "USA", "(206) 555-9857", "5467"));   
